...
Every `CheckSpec` also exposes an `implementation_digest`: a SHA-256 over the
normalized source text of `run`, recorded per check in the decision-contract
manifest.

It closes a gap that declared metadata cannot. Deleting a required gate is caught by
the coverage contract, because the fixed contract still demands the dimension.
Replacing that gate with a permissive implementation that keeps the same ID,
version, coverage, and failure route is not: required coverage still looks
satisfied, and the gate simply stops failing. Before the fingerprint existed, that
substitution produced a byte-identical contract digest. Now it does not.

```python
from agent_economics import implementation_fingerprint

implementation_fingerprint(my_check_function)
```

Scope, stated precisely:

- It detects that an implementation changed. It does not prove that any
  implementation is correct, and it is not a signature.
- Normalization strips the common indent and per-line trailing whitespace, so the
  value depends on source text rather than nesting depth. It is stable across
  CPython versions, which is what keeps published artifacts byte-reproducible on
  the supported matrix.
- Comment and formatting edits inside a check change the digest. That is intended:
  the contract records which code produced a verdict.
- Checks generated by a shared factory share source text and therefore share a
  fingerprint. Closure values are not captured.
- Callables with no retrievable source (built-ins, C functions,
  `functools.partial`, interactively defined functions) raise `ValueError` rather
  than entering a contract with an unbindable implementation.

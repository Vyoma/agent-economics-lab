...
class ContractDigestStabilityTest(unittest.TestCase):
    ...
    Coverage subclasses str, so the enum's digest bytes are unchanged. This
    pins the value under contract schema @2, which binds each check's
    implementation digest as well as its declared identity.
    """

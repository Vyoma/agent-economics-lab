...
| Can a stranger re-derive the evidence? | one upstream verifier per frozen document, 11 of 11 covered | each re-fetches at the pinned revision, checks bytes against the hash the freeze recorded, and re-runs the freezer's own extractor; a document with no verifier fails the run | it proves the frozen file reproduces from the source, not that the source itself is correct; and the sample size per dataset is a knob, so a green run is not a full re-derivation of every row |
| Does it work on data it did not produce? | 10 public datasets audited | 8 with verified findings, 2 clean | an arm name identifies runs in a dataset, never a measurement of a model |
...
| How good is the shipped judge (`kimi-judge@1`)? | agreement with hand-authored rubric-derived labels, 24 constructed cases | 95.8% agreement (23/24), 0/24 false accepts, 95% upper bound 14% (eval-version 1) | not accuracy against production ground truth; constructed cases are easier than real ones, and the later 100% run is excluded here because the set was edited after seeing this judge |

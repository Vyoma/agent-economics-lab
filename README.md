...
**Pointed at somebody else's data, it found two things.** One published model arm
reports a 100% resolution rate from a field its own cross-check never confirmed
once. Two arms turn out to be the same 500 runs, and the outcome label disagrees
with itself on 44 of them. Details below; everything else here is the machinery
that made those visible and the discipline that keeps its own numbers honest.
